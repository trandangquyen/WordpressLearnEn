	...
	public function get_html_questions(){
		...
		if( !empty( $this->questions ) ){				
			...
			if( isset( $this->settings[ 'size' ] ) && $this->settings[ 'size' ] == 'custom' ) {
				...
				$questionsHTML .= '<style>@media screen and (max-width:' . absint( $this->settings[ 'custom_width' ] ) . 'px){.wq_singleQuestionWrapper{width:100%!important;height:auto!important}}'. $css_height .'</style>';
			}
			$style = 'width:' . $width . 'height:'.  $height;
			
			foreach( $this->questions as $key => $question ){
				$position = $key + 1;
				$desc = !empty( $question[ 'desc' ] ) ? '<div class="desc"><div>' . wp_kses_post( $question[ 'desc' ] ) . '</div></div>' : '';
				...
				$questionsHTML .= '<div class="wq_singleQuestionWrapper wq_IsFlip" style="margin-left:auto;margin-right:auto;'.$style.'">
					...
					<div class="front '.$custom_class.'">
						<img style="'.$image_height.'" src="' . esc_url( $question[ 'image' ] ) . '" /><span class="credits">'.esc_html( $question[ 'imageCredit' ] ).'</span>
						<span class="desc">' . __( 'Click to Flip', 'wp-quiz' ) . '</span>
					</div>
					<div class="back '.$custom_class.'"> 
						<img style="'.$image_height.'" src="' . esc_url( $question[ 'backImage' ] ) . '" /><span class="credits">'.esc_html( $question[ 'backImageCredit' ] ).'</span>
						' . $desc . '
					</div></div>';
			}
