...
class WP_Quiz {
	...
	public function get_data_attrs(){
		...
		$data .= 'data-transition_in="' . esc_attr( $this->settings[ 'animation_in' ] ) . '" ';
		$data .= 'data-transition_out="' . esc_attr( $this->settings[ 'animation_out' ] ) . '" ';
		$data .= 'data-correct-answered="0" ';
		$data .= 'data-quiz-pid="' . $this->id . '" ';
		$data .= 'data-share-url="' . esc_url( get_permalink( $post->ID ) ) . '" ';
		$data .= 'data-post-title="' . esc_attr( get_the_title( $post->ID ) ) . '" ';
		$data .= 'data-retake-quiz="' . esc_attr( $this->settings[ 'restart_questions' ] ) . '" ';
		$data .= 'data-question-layout="' . esc_attr( $this->settings[ 'question_layout' ] ). '" ';
		$data .= 'data-featured-image="' . esc_url( wp_get_attachment_url( get_post_thumbnail_id( $post->ID ) ) ) . '" ';
		$data .= 'data-excerpt="' . esc_attr( get_post_field( 'post_excerpt', $this->id ) ) . '" ';
		$data .= 'data-ajax-url="' . esc_url( admin_url( 'admin-ajax.php' ) ) . '" ';
		$data .= 'data-auto-scroll="' . esc_attr( $this->settings[ 'auto_scroll' ] ) . '" ';
	...
	public function get_html_progress_bar(){
		...
		$html[] =			'<span style="background-color:' .esc_attr( $this->settings[ 'bar_color' ] ). '" class="wq_quizProgressValue"></span>';
		$html[] =		'</div>';
		$html[] = 	'</div>';
		$html[] = '<!--// progress bar-->';
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'wp_quiz-front-js', 'wq_l10n', array(
				...
				'captionTrivia' 		=> __( 'You got %%score%% out of %%total%%', 'wp-quiz' ),
				'captionTriviaFB'		=> __( 'I got %%score%% out of %%total%%, and you?', 'wp-quiz' ),
			) 
		);
