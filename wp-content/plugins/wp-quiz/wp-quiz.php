...
class WP_Quiz_Plugin {
	...
	public function add_shortcode_before_editor() {
		...
		if ( 'wp_quiz' === $typenow && isset( $_GET['post'] ) ) {
			echo '<div class="inside"><strong style="padding: 0 10px;">'.__( 'Shortcode:', 'wp-quiz' ).'</strong> <input type="text" value=\'[wp_quiz id="'.esc_attr( trim( $_GET['post'] ) ).'"]\' readonly="readonly" /></div>';
		}
	}
	...
	public function manage_wp_quiz_columns( $column_name, $id ){
		...
		switch ($column_name) {
			case 'shortcode':
				echo '<div class="field"><input type="text" readonly value="[wp_quiz id=&quot;' . esc_attr( $id ) . '&quot;]" onClick="this.select();" style="width:100%;"></div>';
				break;
			case 'type':
