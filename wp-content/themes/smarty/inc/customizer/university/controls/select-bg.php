<?php
if ( ! class_exists( 'STM_Customizer_Background_Control' ) ) {

	class STM_Customizer_Background_Control extends WP_Customize_Control {

		public $type = 'stm-select-bg';
					...
					<div class="select-bg">
						<ul>
							<?php foreach ( $this->choices as $itemName => $itemLabel ): ?>
								<li class="<?php echo esc_attr( $itemName ); ?>" <?php if($itemLabel){ ?>style="background-image: url('<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/frontend-customizer/<?php echo esc_attr( $itemLabel ); ?>')"<?php } ?>>
